...
* **Trigger:** Scheduled to run automatically at 2:00 PM.
...
**Purpose:** Synthesize the cleaned data, fetch audience metrics, and manage human approval.
...
* **Chief Editor AI:** Synthesizes the final newsletter. Configured with strict prompt constraints and a Fallback Model to prevent timeouts.
* **Human-in-the-Loop (HITL):** Sends a draft via Gmail (`sendAndWait`) and pauses execution using a `Wait` node until a human manager clicks "Approve" or "Reject".
* **Fault Tolerance (Backup Editor):** If the Chief Editor fails, the `Error` output branch automatically routes to a Backup Editor AI (using faster, cheaper models) to ensure a draft is always generated.
* **Delivery & Logging:** Dispatches the final HTML newsletter, saves the final log to the database, and notifies the team.

### 3. Global Error Handler (`Error Workflow.json`)
...
* **AI Error Triage:** Passes raw error data (e.g., "gid=0 not found") to an Error Analysis Agent.
...
2. Ensure the **Global Error Handler** is set to `Published` (Active).
3. Update Credentials for Google Sheets, Gmail, and OpenRouter/Mistral.
